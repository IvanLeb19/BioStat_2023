...
food$Category <- food$Category %>% 
  as.factor()
food$`Nutrient Bank Number` <- food$`Nutrient Bank Number` %>% 
  as.factor()
...
for (i in 1:ncol(cleaned_data)){ 
  g <- c(g,sum(cleaned_data[,i] == 0))
}

# Количество 0 в каждой переменной и количество переменных с 0
...
С помощью матрица корреляций удобно суммировать корреляции между всеми переменными в наборе данных. Кроме того, удобно использовать для поиска корреляций между переменными, чтобы избежать мультиколлинеарности при построении регрессии. Позволяет оценить силу связи между перемнными. Однако есть и недостатки, которые надо учитывать, так корреляция не позволяет оценить нелинейную зависимость, не показывает как из перемнных влияет на другую, не означает каузальность между переменными.
...
cleaned_data$Category <- 
  ...
  as.character()
cleaned_data$Category <- 
  ifelse(test = cleaned_data$Category == "Rice", yes = 1,  no = 0) %>% 
  as.numeric()

#Mодель
food_mod <- glm(Category ~Calcium +Sodium+`Vitamin K`+`Vitamin B6`+`Vitamin A and RAE`+ Choline+Selenium+Potassium+`Saturated Fat`+Protein+Copper+Iron+Phosphorus, family = binomial(link = 'logit'), data = cleaned_data)

#Селекция
drop1(food_mod, test="Chisq")
vif(food_mod)

food_mod1 <- glm(Category ~Sodium+Protein+Copper+Iron, family = binomial(link = 'logit'), data = cleaned_data)
drop1(food_mod1, test="Chisq")
vif(food_mod1)
